...
export default async function Page({
  ...
  const d = await getAdminRows("consultation_requests", {
    ...p,
    searchColumn: "full_name",
  });
  return (
    <>
      <h1 className="text-3xl font-bold">Yêu cầu tư vấn</h1>
      <form className="mt-6 flex gap-3 rounded-xl border bg-white p-4">
        ...
        <button className="rounded-lg border px-4 font-semibold">Lọc</button>
      </form>
      <div className="mt-6">
        <RequestTable rows={d.rows} kind="consultation" statuses={statuses} />
      </div>
    </>
  );
}
