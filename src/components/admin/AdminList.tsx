...
export function AdminList({
  ...
  const href = (next: number) =>
    `?page=${next}&q=${encodeURIComponent(q)}&status=${encodeURIComponent(status)}`;
  return (
    <>
      ...
      <form className="mt-6 flex flex-wrap gap-3 rounded-xl border bg-white p-4">
        ...
        <button className="min-h-11 rounded-lg border px-4 font-semibold">
          Lọc
        </button>
      </form>
      ...
      >
        <Link
          ...
          className={`rounded-lg border px-4 py-2 ${page <= 1 ? "pointer-events-none opacity-50" : ""}`}
          href={href(page - 1)}
        >
          Trang trước
        </Link>
        ...
        <Link
          ...
          className={`rounded-lg border px-4 py-2 ${page >= pages ? "pointer-events-none opacity-50" : ""}`}
          href={href(page + 1)}
        >
          Trang sau
        </Link>
