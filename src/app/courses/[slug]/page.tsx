...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> { const { slug } = await params; const course = await getCourseBySlug(slug); if (!course) return { title: "Không tìm thấy khóa học" }; const url = `/courses/${course.slug}`; return { title: `${course.title}`, description: course.shortDescription, alternates: { canonical: url }, openGraph: { title: course.title, description: course.shortDescription, url, type: "website", ...(course.thumbnailUrl ? { images: [{ url: course.thumbnailUrl, alt: `Ảnh khóa học ${course.title}` }] } : {}) } }; }
