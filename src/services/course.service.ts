...
function orderedRows(value: unknown, kind: "content" | "roadmap" | "curriculum"): OrderedContent[] {
  ...
  return value.map((item) => {
    ...
    return {
      ...
      content: kind === "content" && row.content ? String(row.content) : undefined,
...
function mapCourse(row: Row): Course {
  ...
  return {
    ...
    durationText: row.duration_text ? String(row.duration_text) : null, classSizeText: row.class_size_text ? String(row.class_size_text) : null,
    studyFormat: row.study_format as Course["studyFormat"], originalPrice: row.original_price == null ? null : Number(row.original_price),
    price: row.price == null ? null : Number(row.price), priceDisplay: row.price_display ? String(row.price_display) : null,
    currency: String(row.currency ?? "VND"), discountPercent: row.discount_percent == null ? null : Number(row.discount_percent),
    textbookSummary: row.textbook_summary ? String(row.textbook_summary) : null,
    expectedOutcomesSummary: row.expected_outcomes_summary ? String(row.expected_outcomes_summary) : null,
    isFeatured: Boolean(row.is_featured), publishedAt: row.published_at ? String(row.published_at) : null, displayOrder: Number(row.display_order ?? 0),
    audiences: orderedRows(row.course_audiences, "content"), roadmapStages: orderedRows(row.course_roadmap_stages, "roadmap"),
    curriculumItems: orderedRows(row.course_curriculum_items, "curriculum"), outcomes: orderedRows(row.course_outcomes, "content"),
  };
}
...
export async function getPublishedCourses(filters: CourseFilters = {}): Promise<Course[]> {
  ...
  const courses = (data as Row[]).map(mapCourse).filter((course) => !filters.program || course.category.program.slug === filters.program);
