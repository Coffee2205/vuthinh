...
test("giữ nguyên URL đầy đủ và local path", () => {
  assert.equal(
    storage.getPublicMediaUrl("https://cdn.example.com/image.jpg"),
    "https://cdn.example.com/image.jpg",
  );
  assert.equal(
    storage.getPublicMediaUrl("/images/local.webp"),
    "/images/local.webp",
  );
});
...
test("chuyển storage path thành public URL đúng một lần", () => {
  ...
  const url =
    "https://project-ref.supabase.co/storage/v1/object/public/Public-Media/courses/record-id/image.webp";
...
test("trích đúng path từ URL Supabase hiện tại", () => {
  const url =
    "https://project-ref.supabase.co/storage/v1/object/public/Public-Media/posts/post-id/cover.webp";
...
test("không coi URL ngoài hoặc bucket khác là file có thể xóa", () => {
  assert.equal(
    storage.getPublicMediaPath("https://cdn.example.com/image.jpg"),
    null,
  );
  assert.equal(
    storage.getPublicMediaPath(
      "https://project-ref.supabase.co/storage/v1/object/public/other/image.jpg",
    ),
    null,
  );
});
...
test("chỉ bỏ Image Optimizer cho host ảnh ngoài", () => {
  assert.equal(
    storage.shouldBypassImageOptimization("https://cdn.example.com/image.jpg"),
    true,
  );
  assert.equal(
    storage.shouldBypassImageOptimization(
      "https://project-ref.supabase.co/storage/v1/object/public/Public-Media/courses/id/image.webp",
    ),
    false,
  );
  assert.equal(
    storage.shouldBypassImageOptimization("/images/local.webp"),
    false,
  );
});
...
test("validate MIME và giới hạn 5 MB", () => {
  ...
  assert.equal(
    storage.validatePublicImage(
      new File(
        [new Uint8Array(storage.MAX_PUBLIC_IMAGE_BYTES + 1)],
        "large.jpg",
        { type: "image/jpeg" },
      ),
    ),
    "Ảnh vượt quá dung lượng cho phép 5 MB.",
  );
});
