...
function validateImageFile(file: File, list: string, id: string) {
  if (
    ...
    editorError(list, id, "media-type");
  if (file.size > MAX_PUBLIC_IMAGE_BYTES || file.size === 0)
    editorError(list, id, "media-size");
}
...
export async function saveEditor(formData: FormData) {
  ...
  if (!uuidPattern.test(recordId))
    editorError(config.list, requestedId, "validation");
  ...
  for (const field of config.fields) {
    ...
    if (field === "slug" && !slugPattern.test(raw))
      editorError(config.list, requestedId, "slug");
    if (uuidFields.has(field) && !uuidPattern.test(raw))
      editorError(config.list, requestedId, "validation");
    if (urlFields.has(field) && !z.url().safeParse(raw).success)
      editorError(config.list, requestedId, "url");
    if (numberFields.has(field)) {
      const value = Number(raw);
      if (!Number.isFinite(value) || value < 0)
        editorError(config.list, requestedId, "number");
      payload[field] = value;
  ...
  if (
    ...
    editorError(config.list, requestedId, "consent");
  if (
    "status" in payload &&
  ...
  if (config.media && !isNew) {
    const { data, error } = await db
      .from(config.table)
      .select(config.media.field)
      .eq("id", recordId)
      .maybeSingle();
    if (error || !data) editorError(config.list, requestedId, "database");
    oldMedia =
      String((data as Record<string, unknown>)[config.media.field] ?? "") ||
      null;
  }
  ...
  if (config.media && mediaFile) {
    ...
    const uploaded = await uploadPublicImage(
      db,
      mediaFile,
      `${config.media.folder}/${recordId}`,
    );
    if (!uploaded.ok)
      editorError(
        config.list,
        requestedId,
        uploaded.code === "invalid" ? "media-type" : "media-upload",
      );
    uploadedPath = uploaded.path;
    payload[config.media.field] = uploaded.path;
    if (section === "posts" && !payload.cover_image_alt)
      payload.cover_image_alt = String(payload.title ?? "Ảnh bài viết");
  ...
  revalidatePath(config.list);
  revalidatePath(publicPathBySection[section]);
  if (section === "posts" && payload.slug)
    revalidatePath(`/blog/${String(payload.slug)}`);
