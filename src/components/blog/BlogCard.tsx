...
export function BlogCard({ post, headingLevel = "h2" }: { post: BlogPostListItem; headingLevel?: "h2" | "h3" }) {
  ...
  return <article className="flex h-full flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
    <div className="relative aspect-[16/10] bg-gradient-to-br from-blue-100 via-white to-emerald-100">{post.cover_image_url ? <Image src={post.cover_image_url} alt={post.cover_image_alt || `Ảnh bài viết ${post.title}`} fill unoptimized sizes="(min-width:1024px) 33vw,(min-width:640px) 50vw,100vw" className="object-cover" /> : <div className="grid h-full place-items-center p-6 text-center text-sm font-bold uppercase tracking-[.15em] text-brand-blue">Vũ Thịnh · Kiến thức</div>}</div>
