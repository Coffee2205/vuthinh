...
export const editorConfigs = {
  ...
  posts:{table:"blog_posts",title:"bài viết",list:"/admin/posts",fields:["category_id","author_expert_id","title","slug","excerpt","content","cover_image_url","cover_image_alt","reading_time_minutes","seo_title","seo_description","canonical_url","source_title","source_url","source_publisher","source_accessed_at","source_note","is_featured","status","published_at","display_order"],media:{field:"cover_image_url",folder:"post",label:"Ảnh bìa bài viết",aspect:"16/9"}},
...
export interface EditorOption { value: string; label: string }
export interface EditorOptions { categories: EditorOption[]; authors: EditorOption[] }
...
export async function getEditorOptions(section: EditorSection): Promise<EditorOptions | null> {
  ...
  const [categoriesResult, authorsResult] = await Promise.all([
    db.from("blog_categories").select("id,name").eq("is_active", true).order("display_order"),
    db.from("experts").select("id,full_name").eq("is_active", true).order("full_name"),
  ]);
  if (categoriesResult.error || authorsResult.error) throw new Error("ADMIN_EDITOR_OPTIONS_READ");
  return {
    categories: (categoriesResult.data ?? []).map((item) => ({ value: item.id, label: item.name })),
    authors: (authorsResult.data ?? []).map((item) => ({ value: item.id, label: item.full_name })),
  };
}
