          ...
          {programsContent.groups.map((group) => (
            ...
            >
              ...
              <div className="relative flex items-start justify-between gap-5">
                <div>
                  <p
                    className={`text-xs font-bold uppercase tracking-[0.14em] ${group.featured ? "text-blue-100" : "text-emerald-300"}`}
                  >
                    {group.label}
                  </p>
              ...
              <p
                className={`relative mt-5 max-w-xl leading-7 ${group.featured ? "text-blue-50" : "text-slate-300"}`}
              >
                {group.description}
              </p>
              <ul
                className="relative mt-7 flex flex-wrap gap-2"
                aria-label={`Các hướng học ${group.title}`}
              >
