...
export async function updateRequestStatus(formData: FormData) {
      ...
      : kind === "trial"
        ...
        : kind === "consultation"
          ? {
              ...
              schema: consultationStatusSchema,
              path: "/admin/consultations",
            }
          : null;
  if (!config || !/^[0-9a-f-]{36}$/i.test(id)) return;
  const parsed = config.schema.safeParse(status);
  if (!parsed.success) return;
  const db = await createSupabaseAuthServerClient();
  const payload: Record<string, string> = { status: parsed.data };
  ...
  redirect(`${config.path}?updated=1`);
}
