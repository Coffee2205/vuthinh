...
export const metadata: Metadata = createPageMetadata({
  title: "Các khóa học",
  description:
    "Khám phá các khóa học tiếng Trung và chương trình phát triển năng lực tại Vũ Thịnh.",
  path: "/courses",
});
...
export default async function CoursesPage({
  ...
  searchParams: Promise<{ program?: string; sort?: string }>;
}) {
  const params = await searchParams;
  const sort = sorts.includes(params.sort as CourseSort)
    ? (params.sort as CourseSort)
    : "default";
  const [programs, courses] = await Promise.all([
    getActivePrograms(),
    getPublishedCourses({ program: params.program, sort }),
  ]);
  return (
    <main>
        ...
        <Container>
          ...
          <div className="mt-7 max-w-3xl">
            ...
            <p className="mt-5 text-lg leading-8 text-slate-600">
              Thông tin rõ ràng về cấp độ, thời lượng, hình thức và học phí để
              gia đình dễ dàng trao đổi trước khi lựa chọn.
            </p>
          </div>
          <div className="mt-8">
            <CourseFilters
              programs={programs}
              activeProgram={params.program}
              activeSort={sort}
            />
          </div>
