...
export default async function ConsultationPage({ searchParams }: { searchParams: Promise<{ service?: string }> }) {
  ...
  return <main className="bg-slate-50 py-10 sm:py-14 lg:py-18"><Container><Breadcrumbs items={[{ label: "Trang chủ", href: "/" }, { label: "Chuyên gia", href: "/expert" }, { label: "Đăng ký tư vấn" }]} /><div className="mt-7 max-w-3xl"><p className="text-sm font-bold uppercase tracking-[0.14em] text-brand-blue">Tư vấn lộ trình học tập</p><h1 className="mt-4 text-4xl font-bold tracking-tight text-slate-950 sm:text-5xl">Đăng ký tư vấn</h1><p className="mt-5 text-lg leading-8 text-slate-600">Chia sẻ nhu cầu để cùng chuyên gia {expert.full_name} làm rõ điểm xuất phát, mục tiêu và hướng học phù hợp.</p></div>
    ...
    <div className="mt-10 grid gap-8 lg:grid-cols-[.7fr_1.3fr] lg:items-start"><aside className="rounded-2xl bg-brand-blue p-6 text-white"><p className="text-sm font-bold uppercase tracking-wider text-blue-100">Chuyên gia tư vấn</p><h2 className="mt-3 text-2xl font-bold">{expert.full_name}</h2><p className="mt-2 text-blue-100">{expert.professional_title}</p>{selectedService ? <div className="mt-6 rounded-xl bg-white/10 p-4"><p className="text-sm text-blue-100">Dịch vụ đã chọn</p><p className="mt-1 font-bold">{selectedService.name}</p></div> : serviceSlug ? <p className="mt-6 rounded-xl bg-amber-200 p-4 text-sm text-amber-950">Dịch vụ trong đường dẫn không tồn tại hoặc đã ngừng hoạt động. Vui lòng chọn lại.</p> : null}</aside><ConsultationForm expert={expert} services={expert.consultation_services} selectedServiceId={selectedService?.id} /></div></Container></main>;
}
