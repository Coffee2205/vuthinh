...
const navigation = [
  ...
  { href: "/expert", label: "Giảng viên" },
  { href: "/blog", label: "Blog" },
  { href: "/contact", label: "Liên hệ" },
];

const linkStyles =
  "rounded-md px-2 py-2 text-sm font-medium text-white/85 transition-colors hover:bg-white/10 hover:text-brand-yellow focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-yellow";
    ...
    <header className="border-b border-white/10 bg-[#3D3242] shadow-sm">
      <Container className="flex min-h-20 items-center justify-between gap-6 [&>a]:text-brand-yellow">
        ...
        >
          ...
          <Link href="/trial-registration" className="button-primary ml-2">
            Đăng ký học thử
          </Link>
        </nav>
        <details className="group relative lg:hidden">
          ...
          <nav
            className="absolute right-0 top-14 z-50 flex w-72 flex-col rounded-xl border border-white/10 bg-[#3D3242] p-3 shadow-lg"
          ...
          >
            ...
            <Link href="/trial-registration" className="button-primary mt-2">
              Đăng ký học thử
            </Link>
          </nav>
        </details>
