...
export async function createConsultationRequest(input: ConsultationRequestInput) {
  ...
  const { data: expert, error: expertError } = await supabase.from("experts").select("id").eq("id", input.expert_id).eq("is_active", true).maybeSingle();
  if (expertError || !expert) throw new Error("EXPERT_NOT_AVAILABLE");

  if (input.service_id) {
    const { data: service, error: serviceError } = await supabase.from("consultation_services").select("id").eq("id", input.service_id).eq("expert_id", input.expert_id).eq("is_active", true).maybeSingle();
