...
export async function getAdminRows(
  table: Table,
  options: { page: number; q?: string; status?: string; searchColumn: string },
) {
  await requireAdmin();
...
export async function getDashboardData() {
  ...
  const [
    ...
    count("resources"),
    count("success_stories"),
    count("course_registrations", "new"),
    count("trial_registrations", "new"),
    count("consultation_requests", "new"),
  ]);
  return {
    courses,
    publishedCourses,
    ...
    resources,
    stories,
    newRegistrations: registrations + trials,
    consultations,
  };
}
